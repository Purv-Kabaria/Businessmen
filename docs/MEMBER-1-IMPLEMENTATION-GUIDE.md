...
- [ ] **A6** – Field form: same schema as stall; minimal fields; intent tags; optional audio reference (store `audio_local_id` or similar for Member 2).
- [ ] **A7** – Field draft and submit logic (same as stall, `source_mode = "field"`). Use the **same local duplicate check** (all contacts in IndexedDB, stall + field) before saving so a contact already captured at stall is not re-added from field.
