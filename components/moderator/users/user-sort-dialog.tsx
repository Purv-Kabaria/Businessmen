...
type SortField = "createdAt" | "updatedAt" | "email" | "fullName";
...
export interface ModeratorUserSort {
  field: SortField;
  order: SortOrder;
}

interface ModeratorUserSortDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  initialSort?: ModeratorUserSort | null;
  onApply: (sort: ModeratorUserSort) => void;
  onClear: () => void;
}

export function ModeratorUserSortDialog({
  open,
  onOpenChange,
  initialSort,
  onApply,
  onClear,
}: ModeratorUserSortDialogProps) {
  const [field, setField] = useState<SortField>("fullName");
  const [order, setOrder] = useState<SortOrder>("desc");

  const initialState = useMemo<ModeratorUserSort>(() => {
    return (
      initialSort ?? {
        field: "fullName",
        order: "desc",
  ...
  const currentField = sortFields.find(
    (option) => option.value === initialState.field
  );
  const currentOrder = sortOrders.find(
    (option) => option.value === initialState.order
  );
        ...
        <div className="space-y-4 py-2">
          <div className="rounded-lg border border-border/60 bg-muted/40 p-4">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
              <span>{currentField?.label ?? "Full Name"}</span>
