...
import { AdminUserCreateDialog } from "@/components/admin/users/user-create-dialog";
import { AdminUserViewDialog } from "@/components/admin/users/user-view-dialog";
import {
  AdminUserFilterDialog,
  AdminUserFilters,
} from "@/components/admin/users/user-filter-dialog";
import {
  AdminUserSortDialog,
  AdminUserSort,
} from "@/components/admin/users/user-sort-dialog";
import { FilterIcon, SortDescIcon, UserPlus } from "lucide-react";
...
function AdminUsersPageContent() {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      // Sync page if backend adjusts it (e.g. out of bounds)
      // Only generic sync if strictly needed to avoid loops
      const normalizedPage = Math.max(1, pagination.page);
      if (normalizedPage !== page) {
        // Optionally update query, but carefully to avoid loops. 
        // For now, accept backend data for display.
      }
    } catch (error) {
  ...
  }, [page, sortBy, sortOrder, search, roleFilter, router]);
  ...
  const handleApplyFilters = (nextFilters: AdminUserFilters) => {
    setRoleFilter(nextFilters.role || null);
    setPage(1);
  ...
  const handleApplySort = (sort: AdminUserSort) => {
  ...
  const activeSort: AdminUserSort | null = sorting[0]
    ? {
      field: resolveSortField(sorting[0].id) as AdminUserSort["field"],
